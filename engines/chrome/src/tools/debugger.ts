...
export const searchInSources = defineTool({
  ...
  description:
    'Finds a known function name, endpoint, string literal, token, or code pattern in JavaScript loaded by the selected debugger context—the main frame by default, or the frame chosen with select_frame. It searches external, inline/eval, and minified sources in that context without executing or pausing the page, returning 1-based lines plus context-scoped scriptIds; URLs are the preferred selectors for URL-backed matches. Minified single-line bundles are expanded to one match per occurrence, each carrying a 0-based columnNumber (for single-line sources this equals the file character offset accepted by get_script_source offset=) and a hit-centered lineContent preview with lineLength. Use select_frame first for iframe-specific source work, get_script_source for nearby context, save_script_source for a whole bundle, or set_breakpoint_on_text when runtime values are needed. For a known captured request, prefer get_request_initiator before a broad source search.',
  annotations: {
    ...
    matches: zod
      .array(
        zod.object({
          ...
          columnNumber: zod.number().int().optional(),
          lineLength: zod.number().int().optional(),
        }),
      )
      .optional(),
  }),
  ...
  handler: async (request, response, context) => {
    ...
    try {
      ...
      for (const match of displayMatches) {
        ...
        const colInfo =
          match.columnNumber !== undefined ? `@col${match.columnNumber}` : '';
        response.appendResponseLine(`[${scriptId}] ${url}:${lineNum}${colInfo}`);
        ...
        structuredMatches.push({
          ...
          lineNumber: lineNum,
          lineContent: preview,
          ...(match.columnNumber !== undefined
            ? {columnNumber: match.columnNumber, lineLength: match.lineLength}
            : {}),
        });
      }
      ...
      response.appendResponseLine('---');
      response.appendResponseLine(
        'Tip: Use get_script_source(url=..., startLine, endLine) to view full context around a match. Using url is preferred over scriptId as it stays valid across page navigations. For matches with columnNumber on single-line bundles, get_script_source(url=..., offset=<columnNumber>, length=...) reads the exact bytes around the hit.',
      );
      response.setStructuredContent({
        query,
        totalMatches,
