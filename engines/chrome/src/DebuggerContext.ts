...
export interface SearchMatch {
  ...
  lineNumber: number;
  lineContent: string;
  /** Present on expanded multi-hit lines: 0-based char offset of the occurrence within the line. */
  columnNumber?: number;
  /** Full line length when lineContent is a hit-centered preview window. */
  lineLength?: number;
}
...
const MULTI_HIT_LINE_THRESHOLD = 1000;
...
/**
 * CDP Debugger.searchInContent reports at most one match per line, and a minified
 * bundle is a single line — N occurrences collapse into 1 match, losing hit count
 * and positions. Expand long lines into one SearchMatch per occurrence, anchored by
 * an absolute 0-based columnNumber (for single-line sources this equals the file
 * character offset accepted by get_script_source) with a hit-centered preview window.
 */
function expandMultiHitLines(
  matches: SearchMatch[],
  query: string,
  options: {caseSensitive?: boolean; isRegex?: boolean; maxLineLength?: number} = {},
): SearchMatch[] {
  const {caseSensitive = false, isRegex = false, maxLineLength = 150} = options;
  const expanded: SearchMatch[] = [];
  for (const match of matches) {
    if (match.lineContent.length <= MULTI_HIT_LINE_THRESHOLD) {
      expanded.push(match);
      continue;
    }
    let re: RegExp;
    try {
      re = new RegExp(isRegex ? query : escapeRegExp(query), caseSensitive ? 'g' : 'gi');
    } catch {
      expanded.push(match);
      continue;
    }
    const halfWindow = Math.max(40, Math.floor(maxLineLength / 2));
    let hits = 0;
    let m: RegExpExecArray | null;
    while ((m = re.exec(match.lineContent)) !== null) {
      if (m[0].length === 0) {
        re.lastIndex += 1;
        if (re.lastIndex > match.lineContent.length) break;
        continue;
      }
      const start = Math.max(0, m.index - halfWindow);
      const end = Math.min(match.lineContent.length, m.index + m[0].length + halfWindow);
      expanded.push({
        ...match,
        columnNumber: m.index,
        lineLength: match.lineContent.length,
        lineContent: `${start > 0 ? '...' : ''}${match.lineContent.substring(start, end)}${end < match.lineContent.length ? '...' : ''}`,
      });
      if (++hits >= MAX_HITS_PER_LINE) break;
    }
    if (hits === 0) {
      expanded.push(match);
    }
...
export class DebuggerContext {
  ...
  async searchInScripts(
    ...
    return {query, matches: expandMultiHitLines(matches, query, {caseSensitive, isRegex, maxLineLength})};
  }
