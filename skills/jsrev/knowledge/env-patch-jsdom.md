# env-patch-jsdom — jsdom 环境补丁知识库（深度参考）

> 蒸馏自 `hello_js_reverse_skill/references/jsdom-env-patches.md`（15 类补丁，主体）与 `trace/references/env-patch.md`（总纲），jsrev 融合版 2026-08-30。
> 定位：当目标在 jsdom 中以"环境伪装"方式运行时，jsdom 与真实浏览器的系统性差异补丁。按检测影响分三级，所有代码模板可直接复用。仅限授权目标，用户须自行确保已获授权。

## 0. 总纲（先读）

- **入场条件**：已有 JS 入口 + 固定浏览器样本 + 入口函数/触发顺序 + 真实输入输出形状，四者缺一回浏览器观测（见 [env-patch.md](env-patch.md) §0）。
- **最小补丁哲学**：只补被读到的字段，按"能解释第一处分歧"逐个加；两轮不动即停。
- **完成标准**：固定向量 parity + 残余差异诚实清单；helper 加载成功不算完成。
- **沙箱纪律**：`node:vm`/jsdom 都不是安全边界，目标代码在一次性容器里跑（无凭据、受限文件系统、网络按需）。
...
**问题**：jsdom 所有 DOM 方法都是 JS 实现，`toString()` 会暴露完整源码（如 `createElement(localName) { const esValue = ...`），JSVMP 通过此检测判断是否在真实浏览器中。**难度 ★★★★★——jsdom 环境伪装的第一杀手**。
...
补丁不是盲加的——先测出 jsdom 与真实浏览器的差异，再逐项修。

### 浏览器端采集（firefox 引擎）

分 4-5 批用 `evaluate_js` 在真实浏览器中执行采集脚本（每批代码不要太长，避免执行超时）。快速基准可先用 `compare_env`（覆盖主流检测项），它不含的项（Function.toString / Symbol.toStringTag / DOM 布局等）再用分批采集补齐。采集代码模板见 cases/ 的「浏览器指纹采集方法」。

### jsdom 端采集

在 Node.js 中用 `win.eval()` 执行**完全相同**的采集代码：
...
1. `compare_env` → 快速采集浏览器基准环境，判断差异范围。
2. `evaluate_js` 分批采集细粒度环境值（4-5 批），覆盖 compare_env 不含的项。
3. 本地 Node.js 脚本：在 jsdom 中跑相同采集代码 + 生成 diff 报告。
4. 迭代修复：每次修一批差异 → 重新 diff → 直到归零；每轮结果落 `tasks/<id>/runtime-evidence.jsonl`。
5. 端到端验证：jsdom 生成签名 → 真实接口请求 → 确认数据有效（固定向量 parity + fresh replay，见 SKILL.md §7）。
