...
> lineage：蒸馏自 hello_js_reverse_skill `references/path-b-env-emulation.md`（v3.1.0），2026-08-30 平移至 jsrev skill verticals/。工具名按 [../references/tool-matrix.md](../references/tool-matrix.md) 迁移表清零；原外部补丁库引用已并入本文件 §五/§六，过程状态一律落证据工作区 `tasks/<task-id>/`。
>
> **触发条件**：已完成反爬三分法分类（[../SKILL.md](../SKILL.md) §2）且决策树指向路径 B（jsdom/sdenv/vm 沙箱环境伪装）。
>
> **适用边界**：仅限授权目标，用户须自行确保已获授权。
>
> **配套**：[jsvmp.md](jsvmp.md)（识别/决策总纲，`hot_keys` 指导补环境粒度）、[path-a.md](path-a.md)、案例 [../cases/jsvmp-xhr-interceptor-env-emulation.md](../cases/jsvmp-xhr-interceptor-env-emulation.md)、[../cases/jsvmp-dual-sign-xhr-intercept-cacheOpts-jsdom-firefox.md](../cases/jsvmp-dual-sign-xhr-intercept-cacheOpts-jsdom-firefox.md)。
...
适用条件：JSVMP 与浏览器环境深度绑定（环境指纹参与签名哈希）；劫持了请求链路（XHR/fetch 拦截器），签名在内部完成；算法不可直接提取（字节码保护 + 自定义算法）；jsdom 能成功加载并执行 JSVMP 脚本。典型用时 2-4 小时（对比路径 A 的 4-8 小时）。

---

## 二、步骤 0.5：确认签名函数入口（不可跳过）
...
```
search_code(keyword="<签名函数名，如 frontierSign>", script_url="<VMP脚本URL>")
→ 是否有导出签名函数
...
## 三、步骤 1：firefox 引擎环境指纹采集

### 目标

用 firefox 引擎（Camoufox 反检测浏览器）采集真实浏览器的完整环境指纹，作为 jsdom 补丁的基准。

### 采集基准

```
launch_browser()                      # 有头；指纹档位按目标环境选择
...
- Camoufox 的 C++ 引擎级指纹伪装提供「干净基准」，比普通 Chrome 更可靠；`compare_env` 不覆盖 `Function.prototype.toString` / `Symbol.toStringTag` / DOM 布局等细粒度检测，必须用 evaluate_js 分批手动采集（批次 A-D）；
- 采集结果落盘 `tasks/<task-id>/`（作为 jsdom 补丁的对照基准）。

---

## 四、步骤 2：jsdom 环境采集

目标：在本地 Node.js 中创建 jsdom 实例，执行与步骤 1 **完全相同**的采集代码。
...
关键点：jsdom 实例的 `url` 必须设为目标 URL（影响 `document.URL` / `document.domain`）；`pretendToBeVisual: true` 使 `requestAnimationFrame` 可用；`runScripts: 'dangerously'` 允许执行脚本（环境伪装必需）。
...
`window.Notification / Worker / SharedWorker / RTCPeerConnection / AudioContext / OfflineAudioContext / fetch / matchMedia / indexedDB / caches / visualViewport`；`navigator.connection / getBattery / deviceMemory`。

> 若走过第四板斧（[jsvmp.md](jsvmp.md) §8），`hot_keys` 直接给出该 VMP 实际读取的属性集——把"全量 diff"收窄为"只补 hot_keys 显示的属性"。
...
⚠️ Firefox vs Chrome 格式差异：
  ...
  → firefox 引擎（Camoufox）基于 Firefox 内核，必须使用 Firefox 格式
```

### ② navigator 补丁

- `plugins`: 完整 PluginArray/Plugin/MimeType 对象树（length=5）；`webdriver`: false；`userAgentData`: {brands, mobile, platform}；`connection`: {effectiveType:"4g", downlink, rtt}；每项经 markNative 处理。

### ③ window 补丁

- `chrome` 对象 {runtime, app, ...}；30+ API 存根：Notification / Worker / SharedWorker / RTCPeerConnection / AudioContext / OfflineAudioContext / fetch / matchMedia / indexedDB / caches / visualViewport 等；每个存根经 markNative 处理。

### ④ document + performance 补丁

- `hasFocus()` 始终返回 true；`readyState` "complete"；`visibilityState` "visible"；`hidden` false；`performance.timing` 完整 PerformanceTiming 对象；`performance.navigation` {type: 0, redirectCount: 0}。

### ⑤ DOM 布局属性

- offsetHeight/offsetWidth、clientHeight/clientWidth、scrollHeight/scrollWidth 返回非零值；`getBoundingClientRect()` 返回合理的 DOMRect。

### ⑥ Symbol.toStringTag 全面修复

- document → "HTMLDocument"（而非 "Document"）、screen → "Screen"、navigator → "Navigator"、window → "Window"；通过 Object.defineProperty 设置 Symbol.toStringTag。

## 七、步骤 5：内部验证（win.eval）

目标：从 jsdom 内部验证所有检测点通过。**验证代码必须在 jsdom 的 window 上下文中执行（win.eval）**，因为 JSVMP 看到的就是这个上下文。
...
目标：在 jsdom 中加载完整 JSVMP 脚本，触发签名生成，用截获的签名值发起真实接口请求。
...
- ⚠️ 服务端可能**静默拒绝**（HTTP 200 + 空 body，不报错）——这是环境检测失败的信号；间歇性失败 → 检查时间戳相关的环境值是否未补齐；
- 连续 5 次请求全部返回有效数据才算通过；
- 全部证据（签名样本、请求/响应、验证记录）落盘 `tasks/<task-id>/`，按 SKILL.md §7 完成契约做固定向量 parity + fresh replay。
...
> firefox 引擎（Camoufox）基于 Firefox 内核，与 Chrome 在多个环境值上存在差异。jsdom 补丁必须匹配采集基准浏览器的格式。
...
- 目标站点的 JSVMP 若在 Chrome 环境下开发，可能期望 Chrome 格式，但 firefox 引擎采集的基准是 Firefox 格式；
- **原则**：以 firefox 引擎采集的基准为准——其 C++ 引擎级伪装能通过反检测；服务端若明确校验 Chrome 特征（如 `window.chrome` 必须存在），则额外补丁。
...
| 坑 | 问题与信号 | 解决 |
|---|---|---|
| 1. Canvas 存根不真实 | jsdom canvas 返回空白数据，指纹哈希不匹配 | 用 node-canvas / @napi-rs/canvas 提供真实 Canvas 实现，或从 firefox 引擎采集 canvas 指纹后硬编码返回值 |
| 2. native code 多行格式 | markNative 输出了 Chrome 单行格式，但采集基准是 Firefox 多行格式 | 返回值由 `"function name() { [native code] }"` 改为 `"function name() {\n    [native code]\n}"` |
| 3. Symbol.toStringTag 遗漏 | `Object.prototype.toString.call(document)` 返回 `[object Document]` 而非 `[object HTMLDocument]` | 用下方修复片段 |
| 4. 补丁晚于脚本加载 | JSVMP 在加载时就读取环境值，补丁打晚了无效；XHR Hook 的安装顺序也决定能否截获最终 URL | patchEnvironment() 必须在任何 JSVMP 脚本 eval 之前执行 |
| 5. 服务端静默拒绝 | HTTP 200 + 空 body（不报错），看似成功但没数据——签名格式正确但环境指纹不匹配 | 回步骤 3 重新 diff，补齐遗漏的致命级差异 |
| 6. cacheOpts 未传入 | 拦截器不触发、签名不生成（如 `_SdkGlueInit`：旧版只需 bdms.paths，新版必须同时传 cacheOpts） | 确认初始化调用包含 cacheOpts 字段，且 cacheOpts.paths 覆盖目标 API 路径 |
...
1. **环境伪装优先于算法追踪**——深度绑定环境的 VMP，路径 B 通常比路径 A 快。
2. **`Function.prototype.toString` 是 jsdom 环境伪装的第一杀手**——markNative 三层防御必须到位。
...
> 深入阅读：[jsvmp.md](jsvmp.md)（识别/决策/第四板斧 `hot_keys` 精确指导补环境）、[path-a.md](path-a.md)、[../knowledge/invariants.md](../knowledge/invariants.md)。
