...
> lineage: 源自 trace/references/captcha/csdn-point-click-workflow.md，2026-08-30 平移至 jsrev skill verticals/captcha/。
>
> **外部引用标注**：文中引用的案例项目（`<外部项目目录>` 及其 `tools/` 下脚本，如 `tools/csdn_live_verify_flow.py`、`tools/recognize_target_text.py`、`tools/jfbym_points_api.py`）属外部参考项目，**不随包发布**；本文只保留协议流程与参数结论。
