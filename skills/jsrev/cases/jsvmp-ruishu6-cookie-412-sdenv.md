...
> lineage: 源自 hello_js_reverse_skill/cases/jsvmp-ruishu6-cookie-412-sdenv.md，2026-08-30 迁移至 jsrev skill；旧工具名已按 [references/tool-matrix.md](../references/tool-matrix.md) 机械替换。
> 难度：★★★★★
...
快速检测命令（30秒内完成）：
  ...
  - scripts（firefox 源码枚举）→ 存在 230KB+ 文件 → 辅助确认
...
步骤 1: Camoufox 反检测浏览器 + 网络捕获
  启动 jsrev-firefox 引擎（Camoufox，C++ 引擎级指纹伪装），用 inject_hook_preset
  注入 xhr/fetch/crypto 持久化 Hook，导航到目标页面。
...
步骤 7: RS JS 保存
  将 230KB 的RS JS 文件源码落盘到本地（firefox 引擎 `scripts` 枚举，源码读写并入插桩流，见 tool-matrix），分析其结构：
