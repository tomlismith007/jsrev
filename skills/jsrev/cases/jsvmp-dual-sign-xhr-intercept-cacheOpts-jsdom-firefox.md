...
> lineage: 源自 hello_js_reverse_skill/cases/jsvmp-dual-sign-xhr-intercept-cacheOpts-jsdom-firefox.md，2026-08-30 迁移至 jsrev skill；旧工具名已按 [references/tool-matrix.md](../references/tool-matrix.md) 机械替换。
> 难度：★★★★★
...
```
步骤 1: network_capture(action="start")（firefox 需先 start；chrome 急切抓 body）+ 触发业务操作
...
```
步骤 14: 用 jsrev-firefox 引擎启动 Camoufox 反检测浏览器（注意：Camoufox 基于 Firefox）
...
使用 jsrev-firefox 引擎在 Firefox 内核浏览器中采集指纹：
