...
> lineage: 源自 hello_js_reverse_skill/cases/jsvmp-xhr-interceptor-env-emulation.md，2026-08-30 迁移至 jsrev skill；旧工具名已按 [references/tool-matrix.md](../references/tool-matrix.md) 机械替换。
> 难度：★★★★★
...
- [x] debugger 定时器陷阱（firefox 引擎可用 `inject_hook_preset` 的 `debugger_bypass` 预设绕过；chrome 引擎禁 JS 注入，不适用此法）
...
快速检测命令（30秒内完成）：
  ...
  - scripts（firefox 源码枚举）→ 存在 380KB+ 文件 → 辅助确认
...
- **Phase 1**: `search_code(keyword="_SdkGlueInit")` 定位配置；用 firefox 引擎 `scripts` 枚举并把三件套（webmssdk.es5.js / bdms.js / sdk-glue.js）源码落盘到 `config/`（源码读写并入插桩流，见 tool-matrix）
- **Phase 2**: 加载顺序 webmssdk → bdms → sdk-glue → `_SdkGlueInit`；签名在 XHR send 阶段追加
- **Phase 3**: 用 firefox 引擎 `compare_env` + `evaluate_js` 分批采集真实环境，与 jsdom 逐项 diff，**确认需要补的环境项范围后再写补丁**
...
- **SDK 脚本必须保存到本地**（firefox 引擎 `scripts` 枚举后源码落盘），不要每次从 CDN 下载
...
```
步骤 1: network_capture(action="start")（firefox 需先 start；chrome 急切抓 body）+ list_network_requests → 捕获带 a_bogus 的完整请求 URL
...
```
步骤 10: 用 jsrev-firefox 引擎启动 Camoufox 反检测浏览器，navigate 到目标页面
...
使用 jsrev-firefox 引擎在真实浏览器中采集指纹的完整流程：
