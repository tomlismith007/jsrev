/**
 * VM 沙箱执行器（jsrev cli vendor，蒸馏自 hello_js_reverse_skill/scripts/sandbox-runner.js，2026-08-30）
 * 用于在隔离的 Node.js 沙箱中执行从浏览器提取的 JS 代码
 *
 * 已知边界（使用前必读）：
 * - node:vm 不是安全边界——目标 JS 只应在一次性 OS 沙箱/容器中运行不可信样本（见 knowledge/env-patch.md 总纲）。
 * - XHR mock 一律返回 200/'{}'；TextEncoder/URL fallback 是空 class——遇到依赖这些的样本会产生异常行为。
 * - 默认 UA 是 Chrome 但 navigator 带 connection/getBattery：若改为 Firefox UA，须删掉 Chrome 专有字段（UA 自洽纪律）。
 *
 * 使用方式:
